...
class ExadataCollector():
    ...
    def collect(self):
        ...
        for exa_command in list_of_commands.Command.Commands:
            yield self.getExaMetrics(exa_command['osCommand'],exa_command['metricFamilyName'],exa_command['metricFamilyDesc'],exa_command['metricObjName'])
    ...
    def getExaMetrics(self,osCommand,metricFamilyName,metricFamilyDesc,metricObjName):
        ...
        out = subprocess.Popen(osCommand, shell=True,stdout=subprocess.PIPE, stderr=subprocess.STDOUT)

        stdout,stderr = out.communicate()
        # read output command and make some data cleansing
        dt = stdout
        lst = list(item.split(",") for item in dt.replace("b'","").replace("'","").split('\n'))
        # create  metrics family for metrics
        counterFamily = GaugeMetricFamily(metricFamilyName ,metricFamilyDesc, labels=['node','object_name','metric_name'])
        for items in lst[:-2]:
            server = items[0]
            metricName = items[1]

            if metricObjName in ["HOSTINTERCONNECT"]:
            ...
            counterFamily.add_metric([server,metricObjectName,metricName], metricValue)
